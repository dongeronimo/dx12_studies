...
void SetViewportAndScissors(std::shared_ptr<transforms::Pipeline> pipeline, uint32_t W, uint32_t H)
{
    pipeline->viewport.TopLeftX = 0;
    pipeline->viewport.TopLeftY = 0;
    pipeline->viewport.Width = static_cast<LONG>(W);
    pipeline->viewport.Height = static_cast<LONG>(H);
    pipeline->viewport.MinDepth = 0.0f;
    pipeline->viewport.MaxDepth = 1.0f;
    pipeline->scissorRect.left = 0;
    pipeline->scissorRect.top = 0;
    pipeline->scissorRect.right = static_cast<LONG>(W);
    pipeline->scissorRect.bottom = static_cast<LONG>(H);
}
